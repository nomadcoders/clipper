  ...
  return (
    <main className="min-h-screen bg-canvas text-ink">
      <header className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-5 py-5 sm:px-8 sm:py-6">
        ...
        <div className="flex shrink-0 items-center gap-3"><Link href="/#book-prism" className="rounded-full bg-brand px-4 py-2.5 text-sm font-bold !text-white sm:px-5">Book a visit ›</Link><ThemeToggle /></div>
      </header>
        ...
        {appointments.length ? (
          <div className="mt-8 overflow-hidden rounded-2xl border border-line bg-surface shadow-[var(--card-shadow)] sm:mt-12">
            {appointments.map((appointment) => (
              <Link key={appointment.id} href={`/appointments/${appointment.reference}`} className="grid gap-3 border-b border-line-soft p-5 last:border-b-0 hover:bg-surface-2 active:bg-line-soft sm:grid-cols-[1.25fr_1fr_auto] sm:items-center sm:gap-4 sm:p-7">
                <div><p className="text-lg font-bold tracking-[-0.035em] sm:text-xl">{appointment.pet.name} · {appointment.package.name}</p><p className="mt-1 text-sm text-ink-muted">{appointment.neighborhood} · {appointment.groomer?.name ?? "Groomer pending"}</p></div>
