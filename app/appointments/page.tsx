    ...
    <main className="min-h-screen bg-canvas text-ink">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-6 sm:px-8">
        ...
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <Link href="/#book-prism" className="rounded-full bg-accent px-5 py-2.5 text-sm font-bold !text-white">Book a visit ›</Link>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-6xl px-5 pb-24 pt-14 sm:px-8 sm:pt-20">
        ...
        {appointments.length ? (
          <div className="mt-12 overflow-hidden rounded-2xl border border-line bg-surface shadow-[0_28px_80px_rgb(38_46_77/10%)]">
            {appointments.map((appointment) => (
              <Link key={appointment.id} href={`/appointments/${appointment.reference}`} className="grid gap-4 border-b border-line-2 p-5 last:border-b-0 hover:bg-surface-alt sm:grid-cols-[1.25fr_1fr_auto] sm:items-center sm:p-7">
                <div><p className="text-xl font-bold tracking-[-0.035em]">{appointment.pet.name} · {appointment.package.name}</p><p className="mt-1 text-sm text-ink-3">{appointment.neighborhood} · {appointment.groomer?.name ?? "Groomer pending"}</p></div>
