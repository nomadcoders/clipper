...
import { formatDate, formatDateKey, formatMoney, formatTime } from "@/lib/clipper/format";
...
export function BookingFlow({ options, sectionId = "book" }: { options: BookingOption; sectionId?: string }) {
  ...
  const [selectedPackageId, setSelectedPackageId] = useState(() => options.packages[0]?.id ?? "");
      ...
      <section id={sectionId} className="clipper-booking clipper-booking--prism scroll-mt-5">
        <div className="mx-auto max-w-7xl px-5 pb-16 sm:px-8 sm:pb-24 lg:px-12">
          <div className="mb-8 max-w-3xl pt-2 sm:mb-10">
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-ink-muted sm:mt-5">Choose the pet, service, location, and time. We’ll match the route with a trusted Seoul groomer.</p>
          </div>
            ...
            <div className="grid lg:grid-cols-[minmax(0,1fr)_20rem]">
              <div className="space-y-8 p-5 sm:space-y-10 sm:p-8 lg:p-10">
                ...
                <fieldset>
                  ...
                  <div className="grid gap-3 sm:grid-cols-[1.25fr_0.75fr]"><Input aria-label="Street address" value={address} onChange={(event) => setAddress(event.target.value)} placeholder="Street address" className="h-12 rounded-md border-line bg-surface px-4 text-base focus-visible:ring-brand sm:text-sm" /><select aria-label="Neighborhood" value={selectedNeighborhood} onChange={(event) => setSelectedNeighborhood(event.target.value)} className="h-12 rounded-md border border-line bg-surface px-4 text-base outline-none sm:text-sm"><option value="">Neighborhood</option>{neighborhoods.map((neighborhood) => <option key={neighborhood} value={neighborhood}>{neighborhood}</option>)}</select></div>
                  <Input placeholder="Apartment, floor, or gate code (optional)" className="mt-3 h-11 rounded-md border-line bg-surface px-4 text-base sm:text-sm" />
                </fieldset>
