...
export function BrandMark({ className = "", compact = false }: BrandMarkProps) {
  return (
    <span className={`inline-flex items-center gap-2 ${className}`}>
      <span
        aria-hidden="true"
        className="grid size-9 shrink-0 place-items-center rounded-full bg-[#ff6b58] text-[#fff8e9] shadow-[3px_3px_0_#25231e]"
      >
      ...
      <span className={compact ? "sr-only" : "font-serif text-[1.65rem] font-bold leading-none tracking-[-0.07em] text-[#25231e]"}>
        clipper
      </span>
    </span>
  );
